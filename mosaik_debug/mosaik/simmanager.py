...
from ..mosaik_api import (
    __version__,
    check_api_compliance,
    get_wrapper
)

from .exceptions import ScenarioError, SimulationError
from .util import sync_process
...
def start_inproc(world, sim_name, sim_config, sim_id, time_resolution, sim_params):
    ...
    api_compliant = check_api_compliance(sim)
    if api_compliant:
...
class LocalProcess(SimProxy):
    ...
    def _get_proxy(self, methods):
        ...
        """
        proxy_dict = {
            name: get_wrapper(getattr(self._inst, name),
                                         self._world.env)
            for name in methods
        }
